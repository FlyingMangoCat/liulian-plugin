...
const plusPath = `${resPath}/liulian-res-plus/`

const checkAuth = async function (e) {
  return await e.checkAuth({
    auth: 'master',
    replyMsg: `只有主人才能命令榴莲哦~
    (*^ω^*) `
  })
}
...
export async function updateRes (e) {
  ...
  if (fs.existsSync(`${resPath}/miao-res-plus/`)) {
    ...
    command = `git clone https://gitee.com/huifeidemangguomao/liulian.git "${resPath}/liulian-res-plus/"`
    e.reply('开始尝试安装图片加量包，可能会需要一段时间，请耐心等待~')
    exec(command, function (error, stdout, stderr) {
      if (error) {
        e.reply('角色图片加量包安装失败！\nError code: ' + error.code + '\n' + error.stack + '\n 请稍后重试。')
      } else {
        e.reply('角色图片加量包安装成功！您后续也可以通过 #榴莲更新图像 命令来更新图像')
      }
    })
  }
  return true
