...
export async function 哪个群友是我老婆(e) {
    ...
    if (randomWife.last_sent_time>=(new Date().getTime() - 259200000)/1000){
        let msg = [
            ...
            `【${randomWife.nickname}】 (${randomWife.user_id}) 哒哒哒！(健康使用要刷屏！注：五分之一概率没有老婆)`
        ]
        e.reply(msg);
    ...
    if (randomWife3.last_sent_time>=(new Date().getTime() - 259200000)/1000){
        let msg = [
            ...
            `【${randomWife3.nickname}】 (${randomWife3.user_id}) 哒哒哒！(健康使用不要刷屏！注：五分之一概率没有老婆)`
        ]
        e.reply(msg);
    ...
    if (randomWife3.last_sent_time>=(new Date().getTime() - 259200000)/1000){
        let msg = [
            ...
            "今天你的群友老婆是",
            segment.image(`https://q1.qlogo.cn/g?b=qq&s=0&nk=${randomWife3.user_id}`),
            `【${randomWife4.nickname}】 (${randomWife3.user_id}) 啦啦啦！(健康使用不要刷屏！注：五分之一概率没有老婆)`
        ]
        e.reply(msg);
