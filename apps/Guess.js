...
const starpluginName = 'games-template-star-plugin-zolay-liulian';
const starrender = getPluginRender(starpluginName);
init();
const starguessConfigMap = new Map();
function getGuessConfig(e) {
  let key = e.message_type + e[e.isGroup ? 'group_id' : 'user_id'];
  let config = starguessConfigMap.get(key);
  if (config == null) {
    config = {
      playing: false,
      roleId: '',
      timer: null,
      answer: null,
      delete: () => starguessConfigMap.delete(key),
    };
    starguessConfigMap.set(key, config);
  }
  return config;
}
const colors = [// 随机背景颜色
...
export async function guessAvatar(e) {
  ...
  let base64 = null;
  let promise = starrender(templateName, 'question', props);
  setTimeout(async () => {
    base64 = await promise;
    if (base64) {
      e.reply(segment.image(`base64://${base64}`));
      guessConfig.normalMode = normalMode;
      guessConfig.answer = starrender(templateName, 'answer', props);
      guessConfig.timer = setTimeout(() => {
        if (guessConfig.playing) {
          replayAnswer(e, ['很遗憾，还没有人答对哦，正确答案是：' + roleName], guessConfig);
        }
...
function init() {
  let pluginPath = path.join(_path, 'plugins', starpluginName);
  let templatePath = path.join(pluginPath, `resources`, templateName);
  let questionPath = path.join(templatePath, 'question.html');
  let answerPath = path.join(templatePath, 'answer.html');
  if (!fs.existsSync(questionPath)) {
    if (!fs.existsSync(templatePath)) {
      Data.createDir(_path, `/plugins/${starpluginName}/resources/${templateName}`);
    }
    fs.writeFileSync(questionPath, getTemplate());
    fs.writeFileSync(answerPath, getTemplate(false));
  }
