...
export async function weather(e) {
  ...
  console.log(keyword);
  let url = `https://v2.alapi.cn/api/tianqi?token=zO5Dd2HkhbM9Wbyt&city=${keyword}`;
  let response = await fetch(url);
  let res = await response.json(); //结果json字符串转对象
       let msg = [
"城市：",res.data.city,
...
"\n","预警情况：",res.data.alarm,
"\n","可心提示您：今日",res.data.aqi.air_tips,
  ];
...
export async function 早报(e) {
   ...
   let url = `https://v2.alapi.cn/api/zaobao?token=zO5Dd2HkhbM9Wbyt&format=image`;
