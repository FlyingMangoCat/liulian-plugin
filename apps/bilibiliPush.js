...
async function initBiliCookie() {
    ...
    if (fs.existsSync(_path + "/data/PushNews/BilibiliPushConfig.json")) {
      try {
        ...
        if (configData.bilibiliCookie && configData.bilibiliCookie.cookie && configData.bilibiliCookie.cookie.trim() !== '') {
          ...
          Bot.logger?.mark(`[liulian-plugin]B站推送：从配置文件加载 Cookie (长度: ${BiliCookie.length})`);
        }
      } catch (err) {
        Bot.logger?.warn(`[liulian-plugin]B站推送：从配置文件加载 Cookie 失败: ${err.message}`);
      }
    }
  }
  ...
  if (!BiliCookie || BiliCookie.trim() === '') {
    if (Bot?.logger) {
      Bot.logger?.mark('[liulian-plugin]B站推送：未配置有效的 Cookie，B站推送功能无法使用');
      Bot.logger?.mark('[liulian-plugin]B站推送：请主人发送 #B站扫码登录 进行配置');
    }
    return false;
  }
  ...
  if (Bot?.logger) {
    Bot.logger?.mark(`[liulian-plugin]B站推送：使用扫码登录的 Cookie (长度: ${BiliCookie.length})`);
  }
...
async function getLoginUserInfo() {
  ...
  try {
    ...
    if (res.code === -352) {
      Bot.logger?.warn('[liulian-plugin]B站推送：Cookie 已过期或无效，请重新扫码登录');
      return;
    }

    if (res.code !== 0) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：获取登录用户信息失败，错误码: ${res.code}`);
      return;
    }
    ...
    if (data && data.mid) {
      BiliUID = data.mid;
      Bot.logger?.mark(`[liulian-plugin]B站推送：当前登录用户 UID: ${BiliUID}`);
    }
  } catch (err) {
    Bot.logger?.error(`[liulian-plugin]B站推送：获取登录用户信息异常: ${err.message}`);
  }
}
...
async function getUserInfo(uid, retryCount = 0) {
  // 检查是否配置了有效的 Cookie
  if (!BiliCookie || BiliCookie.trim() === '') {
    Bot.logger?.warn('[liulian-plugin]B站推送：未配置有效的 Cookie');
    return null;
  }
  ...
  try {
    ...
    if (res.code === 799) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：接口访问受限（错误码799），第${retryCount + 1}次遇到`);
      // 重试机制：最多重试3次，每次等待5-10秒
      if (retryCount < 3) {
        let retryDelay = Math.floor(Math.random() * 5) + 5;
        Bot.logger?.mark(`[liulian-plugin]B站推送：等待${retryDelay}秒后重试获取用户[${uid}]信息`);
        await common.sleep(retryDelay * 1000);
        return getUserInfo(uid, retryCount + 1);
      } else {
        Bot.logger?.warn('[liulian-plugin]B站推送：达到最大重试次数，仍然受限');
        return { error: 799, message: '接口访问受限' };
      }
    }

    if (res.code !== 0) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：获取用户信息失败，错误码: ${res.code}, 消息: ${res.message}`);
      return null;
    }

    const data = res?.data;
    if (!data) {
      Bot.logger?.warn('[liulian-plugin]B站推送：用户信息数据为空');
      return null;
    }
    ...
    Bot.logger?.error(`[liulian-plugin]B站推送：获取用户信息异常: ${err.message}`);
    return null;
  }
}
...
async function getUserDynamicList(uid, retryCount = 0) {
  ...
  try {
    ...
    if (res.code === 799) {
      ...
      if (retryCount < 3) {
        let retryDelay = Math.floor(Math.random() * 5) + 5;
        Bot.logger?.mark(`[liulian-plugin]B站推送：等待${retryDelay}秒后重试获取用户[${uid}]动态`);
        await common.sleep(retryDelay * 1000);
    ...
    if (res.code !== 0) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：获取用户动态列表失败，错误码: ${res.code}, 消息: ${res.message}`);
      return null;
    }

    return res.data;
  } catch (err) {
    Bot.logger?.error(`[liulian-plugin]B站推送：获取用户动态列表异常: ${err.message}`);
    return null;
  }
}
...
export async function biliLogin(e) {
  ...
  try {
    ...
    try {
      ...
      Bot.logger?.error(`[liulian-plugin]B站推送：生成二维码失败: ${err.message}`);
      ...
      try {
        ...
        if (pollRes.code === 0) {
          ...
          if (dataCode === 0) {
            ...
            // 等待扫码
            if (!isScanned) {
              Bot.logger?.mark("[liulian-plugin]B站推送：等待扫码...");
            }
          } else if (dataCode === 86101) {
            // 已扫码，等待确认
            if (!isScanned) {
              Bot.logger?.mark("[liulian-plugin]B站推送：已扫码，等待确认...");
              isScanned = true;
            }
          }
        }
      } catch (err) {
        Bot.logger?.error(`[liulian-plugin]B站推送：轮询登录状态异常: ${err.message}`);
      }
    ...
    Bot.logger?.error(`[liulian-plugin]B站推送：扫码登录异常: ${err.message}`);
  ...
  try {
    ...
    for (const cookieStr of setCookieHeaders) {
      try {
        ...
        Bot.logger?.warn(`[liulian-plugin]B站推送：设置Cookie失败: ${err.message}`);
      }
    }
    ...
    Bot.logger?.mark(`[liulian-plugin]B站推送：登录成功，DedeUserID=${BiliUID}`);
    ...
    try {
      ...
      Bot.logger?.mark('[liulian-plugin]B站推送：Cookie已保存到配置文件');
    } catch (err) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：保存Cookie到配置文件失败: ${err.message}`);
    }
    ...
    Bot.logger?.error(`[liulian-plugin]B站推送：处理登录成功状态异常: ${err.message}`);
...
export async function changeBilibiliPush(e) {
  ...
  if (e.msg.includes("开启")) {
    ...
    savePushJson();
    Bot.logger?.mark(`[liulian-plugin]开启B站动态推送:${pushID}`);
  ...
  if (e.msg.includes("关闭")) {
    if (PushBilibiliDynamic[pushID]) {
      ...
      savePushJson();
      Bot.logger?.mark(`[liulian-plugin]关闭B站动态推送:${pushID}`);
...
async function processBatchPush(allPushTasks) {
  ...
  for (let task of allPushTasks) {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      if (pushList && pushList.length > 0) {
        ...
        Bot.logger?.mark(`[liulian-plugin]B站推送：用户[${biliUID}]有 ${pushList.length} 条新动态，估算阅读时间=${readingTime}秒`);
        ...
        Bot.logger?.mark(`[liulian-plugin]B站推送：等待${actualDelay}秒后处理下一个用户（基于估算阅读时间${readingTime}秒）`);
        await common.sleep(actualDelay * 1000);
      } else {
        // 没有新动态，使用基础随机延迟30-60秒
        let baseDelay = Math.floor(Math.random() * 30) + 30;
        Bot.logger?.mark(`[liulian-plugin]B站推送：等待${baseDelay}秒后处理下一个用户（无新动态）`);
  ...
  for (let [dynamicId, dynamicData] of allDynamics) {
    ...
    Bot.logger?.mark(`[liulian-plugin]B站推送：开始推送动态[${dynamicIndex}/${allDynamics.size}] [${dynamicId}] 到 ${pushTargets.length} 个群`);
    
    // 构建消息
    let msg = buildSendDynamic(biliUser, dynamic, { sendType: 'default' });
    
    if (!msg || msg === "can't push transmit") {
      Bot.logger?.warn(`[liulian-plugin]B站推送：动态信息解析失败或跳过转发 [${dynamicId}]`);
      continue;
    }
      ...
      try {
        ...
        if (pushInfo.isGroup) {
          await Bot.pickGroup(pushTarget)
            .sendMsg(finalMsg)
            .catch((err) => {
              Bot.logger?.error(`[liulian-plugin]B站推送：群[${pushTarget}]推送失败: ${err.message}`);
        ...
        Bot.logger?.mark(`[liulian-plugin]B站推送：成功推送动态 [${dynamicId}] 到 群[${pushTarget}]`);
        ...
        Bot.logger?.error(`[liulian-plugin]B站推送：发送动态异常 [${dynamicId}]: ${err.message}`);
      }
    }
    ...
    if (dynamicIndex < allDynamics.size) {
      ...
      Bot.logger?.mark(`[liulian-plugin]B站推送：等待${delayBetweenDynamics}秒后推送下一条动态`);
...
async function fetchUserDynamicWithRetry(pushInfo, user, biliUID, retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < maxRetries) {
      Bot.logger?.warn(`[liulian-plugin]B站推送：获取用户[${biliUID}]动态失败，第${retryCount + 1}次重试，错误: ${err.message}`);
      // 失败后等待5-15秒重试
      let retryDelay = Math.floor(Math.random() * 10) + 5;
      await common.sleep(retryDelay * 1000);
      return fetchUserDynamicWithRetry(pushInfo, user, biliUID, retryCount + 1);
    } else {
      Bot.logger?.error(`[liulian-plugin]B站推送：获取用户[${biliUID}]动态失败，已达最大重试次数，错误: ${err.message}`);
...
async function fetchUserDynamic(pushInfo, user, biliUID) {
  ...
  Bot.logger?.mark(`[liulian-plugin]B站推送：准备获取用户[${biliUID}]的动态`);
  Bot.logger?.mark(`[liulian-plugin]B站推送：当前Cookie: ${BiliReqHeaders.cookie ? BiliReqHeaders.cookie.substring(0, 20) + '...' : '未配置'}`);
  ...
  if (res.code === 799) {
    Bot.logger?.warn(`[liulian-plugin]B站推送：获取动态接口访问受限（错误码799），用户[${biliUID}]`);
  ...
  if (data.length === 0) {
    Bot.logger?.mark(`[liulian-plugin]B站推送：用户[${biliUID}]暂无新动态`);
    return [];
  }
  ...
  if (pushList.length > 0) {
    Bot.logger?.mark(`[liulian-plugin]B站推送：用户[${biliUID}]有 ${pushList.length} 条新动态待推送`);
    // 记录每条动态的详细信息
    pushList.forEach((item, index) => {
      let author = item?.modules?.module_author || {};
      let pubTime = author?.pub_ts || 0;
      let pubDate = new Date(pubTime * 1000).toLocaleString('zh-CN');
      Bot.logger?.mark(`[liulian-plugin]B站推送：动态[${index + 1}] ID=${item.id_str}, 发布时间=${pubDate}`);
    });
  }
...
async function sendDynamic(info, biliUser, list) {
  ...
  Bot.logger?.mark(`[liulian-plugin]B站推送[${pushID}] - 用户[${biliUser.name}]`);
  ...
  for (let val of list) {
    try {
      ...
      if (msg === "can't push transmit") {
        Bot.logger?.mark(`[liulian-plugin]B站推送：跳过转发动态 [${val.id_str}]`);
        continue;
      }

      if (!msg) {
        Bot.logger?.warn(`[liulian-plugin]B站推送：动态信息解析失败 [${val.id_str}]`);
        continue;
      }
      ...
      if (info.isGroup) {
        await Bot.pickGroup(pushID)
          .sendMsg(msg)
          .catch((err) => {
            Bot.logger?.error(`[liulian-plugin]B站推送：群[${pushID}]推送失败: ${err.message}`);
      ...
      Bot.logger?.mark(`[liulian-plugin]B站推送：成功推送动态 [${val.id_str}]`);
      ...
      Bot.logger?.error(`[liulian-plugin]B站推送：发送动态异常 [${val.id_str}]: ${err.message}`);
    }
  }
...
async function pushAgain(groupId, msg, retryCount = 0) {
  ...
  if (retryCount >= maxRetries) {
    Bot.logger?.error(`[liulian-plugin]B站推送：群[${groupId}]推送失败，已达最大重试次数`);
    return false;
  }
  ...
  try {
    ...
    Bot.logger?.mark(`[liulian-plugin]B站推送：群[${groupId}]重试推送成功 (第${retryCount + 1}次)`);
    return true;
  } catch (err) {
    Bot.logger?.warn(`[liulian-plugin]B站推送：群[${groupId}]重试推送失败 (第${retryCount + 1}次): ${err.message}`);
  ...
  try {
    ...
    switch (dynamic.type) {
      case "DYNAMIC_TYPE_AV":
        ...
        if (!desc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：视频动态数据不完整 [${dynamic.id_str}]`);
          return null;
        }
      ...
      case "DYNAMIC_TYPE_WORD":
        ...
        if (!opusDesc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：文字动态数据不完整 [${dynamic.id_str}]`);
          return null;
        }
      ...
      case "DYNAMIC_TYPE_ARTICLE":
        ...
        if (!desc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：文章动态数据不完整 [${dynamic.id_str}]`);
          return null;
        }
        pics = [];
      ...
      case "DYNAMIC_TYPE_FORWARD":
        ...
        if (!desc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：转发动态数据不完整 [${dynamic.id_str}]`);
          return null;
        }

        if (!dynamic.orig) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：转发动态缺少原始内容 [${dynamic.id_str}]`);
          return null;
        }
        ...
        if (orig && orig.length) {
          ...
          Bot.logger?.warn(`[liulian-plugin]B站推送：转发动态原始内容解析失败 [${dynamic.id_str}]`);
          return null;
        }
      ...
      case "DYNAMIC_TYPE_LIVE_RCMD":
        ...
        if (!desc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：直播动态数据不完整 [${dynamic.id_str}]`);
          return null;
        }
        try {
          ...
          Bot.logger?.warn(`[liulian-plugin]B站推送：直播动态数据解析失败 [${dynamic.id_str}]`);
          return null;
        }

        if (!desc) {
          Bot.logger?.warn(`[liulian-plugin]B站推送：直播动态播放信息不完整 [${dynamic.id_str}]`);
          return null;
        }
      ...
      default:
        Bot.logger?.warn(`[liulian-plugin]B站推送：未处理的动态类型 [${dynamic.type}] [${dynamic.id_str}]`);
        return null;
    }
  } catch (err) {
    Bot.logger?.error(`[liulian-plugin]B站推送：构建动态消息异常 [${dynamic.id_str}]: ${err.message}`);
    return null;
  }
}
