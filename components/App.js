...
import { Version, LiulianError } from '#liulian'
...
class App {
  ...
  v3App () {
    ...
    let cls = class extends Plugin {
      constructor () {
        super({
          name: `榴莲:${cfg.name || cfg.id}`,
          dsc: cfg.desc || cfg.name || '榴莲插件',
    ...
    for (let key in this.apps) {
      ...
      cls.prototype[key] = async function (e) {
        ...
        try {
          ...
          if (err?.message && (err instanceof LiulianError)) {
            // 处理 LiulianError
      ...
      if (app.yzRule && app.yzCheck) {
        ...
        cls.prototype[yzKey] = async function (e) {
          if (!Version.isLiulian && !app.yzCheck()) {
            return false
          }
          e = this.e || e
          e.original_msg = e.original_msg || e.msg
